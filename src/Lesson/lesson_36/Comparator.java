...
public class Comparator {
    public static void main(String[] args) {
        ...
        Arrays.sort(cars, new java.util.Comparator<Car>() {
            @Override
...
//                return (c1.getYear() - c2.getYear()) * (-1); // Тоже будет работать, но так не пишут
        ...
        Функциональный интерфейс - это интерфейс, который имеет РОВНО ОДИН абстрактный метод
         (метод который нужно реализовать, если мы имплементирует интерфейс)
        ...
        java.util.Comparator<Car> carComparator = (c1, c2) -> {
...
//         Статические методы интерфейса Comparator
        java.util.Comparator<Car> byModel = java.util.Comparator.comparing(Car::getModel);

        Arrays.sort(cars, byModel);
        System.out.println(Arrays.toString(cars));

        Arrays.sort(cars, java.util.Comparator.comparing(Car::getYear));
        System.out.println(Arrays.toString(cars));

        // Отсортировать по году выпуска в порядке убывания
        Arrays.sort(cars, java.util.Comparator.comparing(Car::getYear, java.util.Comparator.reverseOrder()));
        // Альтернативный способ
        Arrays.sort(cars, java.util.Comparator.comparing(Car::getYear).reversed());

        System.out.println("===================\n");

        // Отсортируй по модели. Если модели равны, то отсортируй такие машины по убыванию года выпуска
        Arrays.sort(cars, java.util.Comparator.comparing(Car::getModel).thenComparing(Car::getYear, java.util.Comparator.reverseOrder()));
        System.out.println(Arrays.toString(cars));
        ...
        Arrays.sort(cars, java.util.Comparator.comparing(Car::getYear).thenComparing(Car::getModel, java.util.Comparator.reverseOrder()));
        for (Car car : cars) {
...
//        System.out.println(Arrays.toString(cars));
        ...
        Arrays.sort(cars, java.util.Comparator.comparing(Car::getYear).thenComparing(Car::getMaxSpeed).thenComparing(Car::getModel));
